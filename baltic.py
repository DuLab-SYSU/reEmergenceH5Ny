...
class tree: ## tree class
    ...
    def setAbsoluteTime2(self,date):
        """ place all objects in absolute time by providing the date of the most recent tip """
        for i in self.Objects: ## iterate over all objects
            i.absoluteTime=date-i.traits['height'] ## heights are in units of time from the root
        self.mostRecent=max([k.absoluteTime for k in self.Objects])

    def setAbsoluteTime3(self,date):
        """ place all objects in absolute time by providing the date of the most recent tip """
        for i in self.Objects: ## iterate over all objects
            try:
                i.absoluteTime=date-i.traits['height_median'] ## heights are in units of time from the root
            except KeyError:
...
def loadNexus2(tree_path,tip_regex='\|([0-9]+\-[0-9]+\-[0-9]+)',date_fmt='%Y-%m-%d',treestring_regex='tree [A-Za-z\_]+([0-9]+)',variableDate=True,absoluteTime=True,verbose=False):
    """
    Load nexus file
    """
    tipFlag=False
